...
| 57 | Log the lookup ordinal and its 9.x mapping | 3 | same ordinals and mappings as the emulator, exactly | **Root cause, and it is ours.** Three of the game's euser lookups are translated through the **efsrv** table, because `gate6_library_lookup` identifies the library by comparing a stale `RLibrary` pointer. The game is handed `CArrayFixBase::CArrayFixBase` where it asked for `RPointerArrayBase::BinarySearchUnsigned` |
