...
extern "C" void gate6_thread_created(u32 err, const u32 *frame, Context *c)
{
    ...
    if (!frame)
        return;
    // r0 the object, r1 the name, r2 the function, r3 the stack size, then the
    // three the caller pushed: with the 289 signature they are the allocator,
    // the argument and the owner type.
    for (u32 i = 0; i < 4; i++)
        log_event(c, NOTE_THREAD_ARG, frame[i]);
    for (u32 i = 0; i < 3; i++)
        log_event(c, NOTE_THREAD_ARG, frame[FRAME_STACKARG + i]);
    log_event(c, NOTE_THREAD_HANDLE, frame[0] ? ((const u32 *)frame[0])[0] : 0);
    // And the name itself, which the kernel could make nothing of.
    const u32 *name = (const u32 *)frame[1];
    if (name)
        for (u32 i = 0; i < 3; i++)
            log_event(c, NOTE_THREAD_NAME, name[i]);
    log_block(c);
}
...
// Two of the three arguments the kernel receives are junk while the third is
// sound, which is what arguments landing in the wrong places looks like. So log
// the whole frame: the object, the name descriptor and what it points at, the
// function, the stack size, and the three words the caller pushed.
//
//   stmdb sp!, {r0-r4, r12, lr}     @ seven words, then the caller's stack args
//   ldr   r12, =target ; blx r12
//   mov   r4, r0                    @ keep the result across the handler
//   mov   r1, sp                    @ the frame
//   ldr   r2, =ctx ; ldr r12, =handler ; blx r12
//   mov   r0, r4 ; str r0, [sp]     @ the result survives the ldmia
//   ldmia sp!, {r0-r4, r12, lr} ; bx lr
enum { FRAME_THUNK_BYTES = 15 * 4 };

static u32 frame_thunk(u8 *code, const void *ctx, u32 target, u32 handler)
{
    u32 *b = (u32 *)code;
    b[0]  = 0xE92D501F;
    b[1]  = 0xE59FC024;
...
// Where the picture goes, and how big.
//
// Round 74 corrected the shape. Round 72 read the 1280-byte line as "the
// buffer is 320 pixels wide and the reported 240x320 is its rotation", and
// built the layout on that. The phone says otherwise: at 203 pixels wide
// starting at column 58 the picture ran off the right-hand edge of the
// screen and the overflow came back through the left, which only happens if
// the visible width really is the **240** that `UserSvr::ScreenInfo`
// reports. So 1280 bytes is a padded stride -- 240 pixels of four bytes is
// 960, and the hardware keeps a 320-pixel line -- and the reported size is
// the screen. The pitch is for addressing rows and for nothing else.
//
// `clearPending` blanks the framebuffer once after any change, so nothing of
// the phone's own menu is left showing round the picture. With `stretch` the
...
extern "C" void gate6_screen_update(void *self, const void *region, Context *c)
{
    ...
    if (c->clearPending && c->realScreen && c->bufH) {
        ...
        // The buffer is only guaranteed as far as the last row's last
...
extern "C" u32 gate6_library_lookup(void *lib, int ordinal, Context *c)
{
    ...
    if (WRAP_CREATE && fn && kind == LIB_EUSER && mapped == NEW_RTHREAD_CREATE) {
        if (!c->threadCreateThunk && c->spare + FRAME_THUNK_BYTES <= c->spareEnd) {
            ...
            c->spare += FRAME_THUNK_BYTES;
        }
        if (c->threadCreateThunk && ((u32 *)c->threadCreateThunk)[12] == fn)
...
extern "C" u32 gate6_control_offerkey(void *, const void *key, u32 type, Context *c)
{
    if (key) {
        ...
        if (SCREEN_PICKER && type == (u32)EEventKey) {
            ...
            if (code == (u32)KEY_FMT_DEPTH) {   // the one format escape hatch
