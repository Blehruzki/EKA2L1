...
// which is `delete` on the name buffer. A free only faults on a heap that is
// already wrong, so: every free is matched against the allocations still
// outstanding. A pointer that was never handed out, or handed out and freed
// already, is the answer; a pointer that matches cleanly says the damage is
// somewhere else and this free is just where it shows.
